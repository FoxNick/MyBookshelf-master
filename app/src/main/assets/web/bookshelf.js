...
var apiAddress1 = (apiName, url, noteurl, index, name) => {
    let address = $('#address').value || window.location.host;
    if (!(/^http|^\/\//).test(address)) {
        address = "//" + address;
    }
    ...
    localStorage.setItem('address', address);
    return address + apiMap[apiName] + (url ? "?url=" + encodeURIComponent(url) : "")+ (noteurl ? "&noteurl=" + encodeURIComponent(noteurl) : "")+ (index ? "&index=" + encodeURIComponent(index) : "")+ (name ? "&name=" + encodeURIComponent(name) : "");
};
var init = () => {
                    ...
                    fetch(apiAddress("getChapterList", book.noteUrl), { mode: "cors" })
                        ...
                        .then(data => {
                            ...
                            data.data.forEach(chapter => {
								...
								ch.setAttribute("noteurl", chapter.noteUrl);
								ch.setAttribute("index", chapter.durChapterIndex);
...
$('#chapter').addEventListener("click", (e) => {
    if (e.target.tagName === "BUTTON") {
		...
		var noteurl = e.target.getAttribute("noteurl");
		var index = e.target.getAttribute("index");
        if (!url) {
            alert("未取得章节地址");
        }
        $("#content").innerHTML = "<p>" + name + " 加载中...</p>";
        fetch(apiAddress1("getBookContent", url, noteurl, index, name), { mode: "cors" })
            .then(res => res.json())
            .then(data => {
                if (!data.isSuccess) {
                    alert(data.errorMsg);
